...
<head>
<meta charset="UTF-8"/>
<title>Tablero de Priorización de Hipótesis — Altos de Fontibón</title>
...
<body>
  <header>
    <h1>Tablero de Priorización — Riesgo Vehicular Altos de Fontibón</h1>
    <p>Hipótesis del déficit de parqueaderos → IRSV (Índice de Riesgo en Seguridad Vehicular). Gestiona, visualiza y prioriza tus hipótesis en 3 vistas: Tabla, Kanban y Dashboard.</p>
  </header>
  ...
  <main>
      ...
      <div class="panel">
        ...
        <div class="controls">
          <div><label>Estado</label>
            <select id="fEstado"><option value="">Todos</option><option value="v">Verde</option><option value="a">Amarillo</option><option value="r">Rojo</option></select>
          </div>
          ...
          <div><label>Fase DT</label>
            <select id="fFase"><option value="">Todas</option><option value="Empatizar">Empatizar</option><option value="Definir">Definir</option><option value="Idear">Idear</option></select>
          </div>
        ...
        <div class="table-wrap">
          ...
          <div class="empty" id="noRes" hidden>No hay hipótesis que coincidan con los filtros.</div>
        </div>
      </div>
    </section>
    ...
    <section class="view" id="view-kanban">
      <div class="panel">
        ...
        <div class="sub">Arrastra una tarjeta entre columnas para actualizar su Estado (V/A/R) al avanzar en la validación. <strong>V</strong>=lista para correr hoy · <strong>A</strong>=falta 1[...]
        <div class="kanban">
    ...
    <div class="modal">
      ...
      <div class="form-grid">
        ...
        <div><label>Fase DT origen</label>
          <select id="f_fase"><option>Empatizar</option><option>Definir</option><option>Idear</option></select>
        </div>
        ...
        <div><label>Estado</label>
          <select id="f_estado"><option value="v">Verde</option><option value="a">Amarillo</option><option value="r">Rojo</option></select>
        </div>
      </div>
...
<script>
  ...
  const seedData = [
    {
      id:"H-01",
      fase:"Empatizar",
      causa:"Uso de parqueaderos alternativos por déficit físico real de cupos dentro del conjunto. Si se integra la comunidad completa al sistema de asignación de espacios de parqueaderos, en[...]
    ...
    },
    {
      id:"H-02",
      fase:"Definir",
      causa:"Si se reasignan los cupos subutilizados de visitantes hacia un modelo rotativo para residentes con carros en vía pública, entonces el IRSV se reducirá en un 20% en el próximo tri[...]
  ...
  function renderTable(){
    ...
    list.forEach(h=>{
      ...
      tr.innerHTML = `
        <td><strong>${esc(h.id)}</strong><br><span style="font-size:.6rem;color:#66727f;">Score ${h.score}</span></td>
        <td><span class="categoria">${esc(h.fase)}</span></td>
        <td>${esc(h.causa)}</td>
        <td>${esc(h.metrica)||'<span style="color:#aab3bd">— sin definir —</span>'}</td>
        <td><span class="impacto ${h.impacto}">${cap(h.impacto)}</span></td>
        <td style="font-weight:700;color:#2d3a46;">${h.score}</td>
        <td><span class="status ${h.estado}">${estadoLabel(h.estado)}</span></td>
        <td><div><strong>P:</strong> ${esc(h.patron)||'—'}</div><div><strong>R:</strong> ${esc(h.refutacion)||'—'}</div></td>
        <td><strong>✅</strong> ${esc(h.accionConfirma)||'—'}</td>
        <td><strong>❌</strong> ${esc(h.accionRefuta)||'—'}</td>
        <td>${esc(h.valorUsuario)||'—'}</td>
  ...
  function openNew(){
    ...
    ["f_id","f_causa","f_insight","f_metrica","f_patron","f_refutacion","f_accionC","f_accionR","f_valor"].forEach(id=>document.getElementById(id).value="");
